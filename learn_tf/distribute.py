...
def main(_):
    ...
    if FLAGS.job_name == "ps":
        ...
        with tf.device(tf.train.replica_device_setter(worker_device="/job:worker/task:%d" % FLAGS.task_index,
            ...
            weight = tf.get_variable('weight', [1], dtype=tf.float32, initializer=tf.random_normal_initializer())
            biase = tf.get_variable('biase', [1], dtype=tf.float32, initializer=tf.zeros_initializer)
            pred = tf.mul(input, weight) + biase

            loss_value = tf.square(label - pred)
        ...
        with sv.prepare_or_wait_for_session(server.target) as sess:
            ...
            while step < 100000000:
                ...
                if step % steps_to_validate == 0:
                    ...
                    print("step: {}, weight: {}, biase: {}, loss: {}".format(step, w, b, loss_v))
        sv.stop()
